...
import com.github.stazxr.zblog.base.domain.enums.LoginChan;
import com.github.stazxr.zblog.base.domain.enums.LoginType;
import com.github.stazxr.zblog.base.domain.enums.MailTemplate;
import com.github.stazxr.zblog.base.domain.error.UserErrorCode;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public UserVo queryUserDetail(Long userId) {
        ...
        return ThrowUtils.requireNonNull(userVo, BaseErrorCode.ECOREA001);
    }
    ...
    public void addUser(UserDto userDto) {
        ...
        // 新增时，不允许传入 UserId
        ThrowUtils.when(user.getId() != null).system(BaseErrorCode.SCOREB001);
        // 设置用户默认值
        ...
        checkUser(user);
        // 新增用户
        ThrowUtils.when(!save(user)).system(BaseErrorCode.SCOREA001);
        // 保存角色信息
    ...
    public void editUser(UserDto userDto) {
        ...
        ThrowUtils.throwIfNull(dbUser, BaseErrorCode.ECOREA001);
        // 判断用户名是否被修改
        ThrowUtils.throwIfNonEquals(dbUser.getUsername(), user.getUsername(), UserErrorCode.EUSERA009);
        // 权限检查
        checkPermission(dbUser);
        // 用户信息检查
        checkUser(user);
        // 编辑用户
        ThrowUtils.when(!updateById(user)).system(BaseErrorCode.SCOREA002);
        // 保存角色信息
    ...
    public void deleteUser(Long userId) {
        ...
        ThrowUtils.when(userId.equals(loginUser.getId())).service(UserErrorCode.EUSERA007);
        // 权限检查
        checkPermission(dbUser);
        // 删除用户
        ThrowUtils.when(!removeById(userId)).system(BaseErrorCode.SCOREA003);
        // 删除角色信息
    ...
    private void checkUser(User user) {
        ...
        ThrowUtils.throwIf(!RegexUtils.match(username, RegexUtils.Regex.USERNAME_REGEX), UserErrorCode.EUSERA001);
        ThrowUtils.throwIf(checkUsernameExist(user), UserErrorCode.EUSERA000);

        // 邮箱检查
        String email = user.getEmail().trim();
        user.setEmail(email);
        ThrowUtils.throwIf(!RegexUtils.match(email, RegexUtils.Regex.EMAIL_REGEX), UserErrorCode.EUSERA003);
        ThrowUtils.throwIf(checkEmailExist(user), UserErrorCode.EUSERA002);

        // 昵称检查
        AtomicInteger count = new AtomicInteger(1);
        while (checkNicknameExist(user)) {
            if (count.get() < Byte.MAX_VALUE) {
                user.setNickname(user.getNickname() + count.getAndIncrement());
            } else {
                ThrowUtils.service(UserErrorCode.EUSERA004);
            }
        }

        // 账号过期时间检查
        if (UserType.TEMP_USER.getType().equals(user.getUserType())) {
            ThrowUtils.throwIfNull(user.getExpireTime(), UserErrorCode.EUSERA005);
        } else {
    ...
    private void checkPermission(User user) {
        ...
        if (UserType.NORMAL_USER.getType().equals(loginUser.getUserType())) {
            if (UserType.ADMIN_USER.getType().equals(newUserType) || UserType.SYSTEM_USER.getType().equals(newUserType)) {
                ThrowUtils.service(UserErrorCode.EUSERA006);
            }
        }
        // 临时用户：只能新增或编辑临时用户
