      ...
      <div class="search-opts">
        <muses-search-form ref="searchForm" :model="filters" label-position="right" label-width="0" :offset="0" :item-width="140">
          <muses-search-form-item label="" prop="search-content">
            <el-input id="search-content" v-model="filters.content" clearable placeholder="留言内容" @keyup.enter.native="search" />
...
<script>
import detailDialog from '@/views/admin/audit/barrageMessage/template/detailDialog'
import auditDialog from '@/views/admin/audit/barrageMessage/template/auditDialog'
export default {
  name: 'BarrageMessage',
  components: {
    detailDialog,
  ...
  methods: {
    ...
    listTableData() {
      ...
      this.$mapi.barrageMessage.pageBarrageMessageList(param).then(res => {
        ...
        this.row = null
        this.$refs.barrageMessageTable.setCurrentRow()
      })
    },
    // 详情
    showDetail() {
      if (this.row === null) {
        this.$message.error('请选择要查看的留言')
        return
      }
    ...
    deleteBarrageMessage() {
      if (this.row === null) {
        this.$message.error('请选择要删除的留言')
        return
      }
      this.$confirm('此操作将永久删除留言, 是否继续?', '提示', {
