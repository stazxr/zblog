      ...
      >
        ...
        <el-table-column :show-overflow-tooltip="true" prop="execResult" label="请求结果" align="center" width="100">
          <template v-slot="scope">
            ...
            <el-link v-else-if="hasPerm('LOGIQ002')" type="danger" @click="showDetail(scope.row.id)">
...
import detailDialog from '@/views/admin/monitor/interfaceLog/template/showLogDetail'
...
export default {
  ...
  methods: {
    ...
    listTableData() {
      ...
      this.$mapi.log.pageInterfaceLogList(param).then(res => {
        const { data } = res
    ...
    // 删除
    deleteLog() {
      this.$confirm('确认根据查询条件删除接口日志吗, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteLoading = true
        this.$mapi.log.deleteInterfaceLog({ ... this.filters }).then(res => {
    ...
    exportData() {
      this.$confirm(`确认导出接口日志列表吗?`, '提示', {
        ...
        this.$mapi.log.exportInterfaceLog({ ... this.filters }).then(res => {
          downloadFile(res, '接口日志列表.xlsx')
        }).finally(_ => {
